...
export class CartState {
    ...
    if(action.payload.id){
      ...
      let result: CartModel = {
        items: [{
          ...
          sub_total: price ? price * action.payload.quantity : 0,
  ...
  update(ctx: StateContext<CartStateModel>, action: UpdateCart) {
    ...
    cart[index].sub_total = cart[index].quantity * (cart[index]?.variation ? cart[index]?.variation?.price : cart[index].product.price);
